...
function body_request1($size){
    ...
    $body_request2 = '{<br>' . $body_request2 . '}';
...
function database_query4($size){
    ...
    $query = 'SELECT '.$atrs.' FROM '.$_POST['nome_tabela'];
    return $query;
}

function body_response1($size){
    $response = '[<br>  '.body_request1($size).'<br>]';
    return $response;
}

function database_query5($size){
    $query = database_query4($size).' WHERE id_'.$_POST['nome_uri'].'=1';
    return $query;
}